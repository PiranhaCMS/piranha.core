...
/// </summary>
public static class AeroDistributedCacheExtensions
{
    /// <summary>
    ...
    public static IServiceCollection AddAeroDistributedCache(this IServiceCollection services)
    {
        // Check dependent services
        if (!services.Any(s => s.ServiceType == typeof(IDistributedCache)))
        {
            throw new NotSupportedException("You need to register a IDistributedCache service in order to use Memory Cache in Aero.Cms");
        }
    ...
    public static AeroServiceBuilder UseDistributedCache(this AeroServiceBuilder serviceBuilder, bool clone = false)
    {
        serviceBuilder.Services.AddAeroDistributedCache();
