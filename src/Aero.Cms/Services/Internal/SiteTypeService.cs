...
internal sealed class SiteTypeService : ISiteTypeService
{
    ...
    public async Task SaveAsync(SiteType model)
    {
        ...
        if (_cache != null)
        {
            await _cache.RemoveAsync("Aero_SiteTypes").ConfigureAwait(false);
        }
    }
    ...
    private async Task<IEnumerable<SiteType>> GetTypes()
    {
        if (_cache != null)
        {
            var types = await _cache.GetAsync<IEnumerable<SiteType>>("Aero_SiteTypes").ConfigureAwait(false);

            if (types == null)
            {
                types = await _repo.GetAll().ConfigureAwait(false);

                await _cache.SetAsync("Aero_SiteTypes", types).ConfigureAwait(false);
            }
            return types;
        }
        return null;
