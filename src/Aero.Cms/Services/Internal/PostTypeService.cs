...
internal sealed class PostTypeService : IPostTypeService
{
    ...
    public async Task SaveAsync(PostType model)
    {
        ...
        if (_cache != null)
        {
            await _cache.RemoveAsync("Aero_PostTypes").ConfigureAwait(false);
        }
    }
    ...
    private async Task<IEnumerable<PostType>> GetTypes()
    {
        if (_cache != null)
        {
            var types = await _cache.GetAsync<IEnumerable<PostType>>("Aero_PostTypes").ConfigureAwait(false);

            if (types == null)
            {
                types = await _repo.GetAll().ConfigureAwait(false);

                await _cache.SetAsync("Aero_PostTypes", types).ConfigureAwait(false);
            }
            return types;
        }
        return null;
