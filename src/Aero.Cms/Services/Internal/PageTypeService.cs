...
internal sealed class PageTypeService : IPageTypeService
{
    ...
    public async Task SaveAsync(PageType model)
    {
        ...
        if (_cache != null)
        {
            await _cache.RemoveAsync("Aero_PageTypes").ConfigureAwait(false);
        }
    }
    ...
    private async Task<IEnumerable<PageType>> GetTypes()
    {
        if (_cache != null)
        {
            var types = await _cache.GetAsync<IEnumerable<PageType>>("Aero_PageTypes").ConfigureAwait(false);

            if (types == null)
            {
                types = await _repo.GetAll().ConfigureAwait(false);

                await _cache.SetAsync("Aero_PageTypes", types).ConfigureAwait(false);
            }
            return types;
        }
        return null;
