...
public class ModelLoader : IModelLoader
{
        ...
        where T : PageBase
    {
        ...
        if (draft)
        {
            ...
            if ((await _auth.AuthorizeAsync(user, Aero.Cms.Security.Permission.PagePreview)).Succeeded)
            {
        ...
        where T : PostBase
    {
        ...
        {
            // Check that the current user is authorized to preview pages
            if ((await _auth.AuthorizeAsync(user, Aero.Cms.Security.Permission.PostPreview)).Succeeded)
            {
                // Get the draft, if available
