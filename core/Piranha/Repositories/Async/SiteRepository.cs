...
namespace Piranha.Repositories.Async
{
    public class SiteRepository : BaseRepository<Site>, ISiteRepository
    {
        ...
        public async Task<Site> GetByInternalId(string internalId, IDbTransaction transaction = null)
        {
            ...
            {
                model = await conn.QueryFirstOrDefaultAsync<Site>($"SELECT * FROM [{table}] WHERE [InternalId]=@InternalId",
        ...
        public async Task<Site> GetDefault(IDbTransaction transaction = null)
        {
            Site model = cache != null ? cache.Get<Site>($"Site_{Guid.Empty}") : null;

            if (model == null)
            {
                model = await conn.QueryFirstOrDefaultAsync<Site>($"SELECT * FROM [{table}] WHERE [IsDefault]=1",
        ...
        public async Task<Models.Sitemap> GetSitemap(string id = null, bool onlyPublished = true, IDbTransaction transaction = null)
        {
            if (id == null)
            {
                var site = await GetDefault(transaction);
            ...
            if (id != null)
            {
                ...
                if (sitemap == null)
                {
                    var pages = await conn.QueryAsync<Page>("SELECT [Id], [ParentId], [SortOrder], [Title], [NavigationTitle], [Slug], [IsHidden], [Published], [Created], [LastModified] FROM [Piranha_Pages] WHERE [SiteId]=@Id ORDER BY [ParentId], [SortOrder]",
                        new { Id = id }, transaction: transaction);

                    if (onlyPublished)
                        pages = pages.Where(p => p.Published.HasValue);
                    sitemap = await Sort(pages);
        ...
        /// <param name="transaction">The optional transaction</param>
        public override async Task Delete(string id, IDbTransaction transaction = null)
        {
            // Delete all pages within the site
            var pages = await conn.QueryAsync<string>($"SELECT [Id] FROM [Piranha_Pages] WHERE [SiteId]=@Id",
                new { Id = id }, transaction: transaction);
            ...
            await base.Delete(id, transaction);
        }
        ...
        /// <param name="transaction">The optional transaction</param>
        protected override async Task Add(Site model, IDbTransaction transaction = null)
        {
            ...
            if (model.IsDefault)
            {
                ...
                var def = await GetDefault(transaction);

                if (def != null && def.Id != model.Id)
                {
                    def.IsDefault = false;
                    await Update(def, transaction, false);
                }
            }
            else
            {
                // Make sure we have a default site
                var count = await conn.ExecuteScalarAsync<int>($"SELECT COUNT(*) FROM [{table}] WHERE [IsDefault]=1");
                if (count == 0)
                    model.IsDefault = true;
            }
            await conn.ExecuteAsync($"INSERT INTO [{table}] ([Id], [InternalId], [Title], [Description], [Hostnames], [IsDefault], [Created], [LastModified]) VALUES (@Id, @InternalId, @Title, @Description, @Hostnames, @IsDefault, @Created, @LastModified)",
        ...
        /// <param name="model">The model</param>
        /// <param name="transaction">The optional transaction</param>
        protected override async Task Update(Site model, IDbTransaction transaction = null)
        {
            await Update(model, transaction, true);
        }
        ...
        protected async Task Update(Site model, IDbTransaction transaction = null, bool checkDefault = true)
        {
            ...
            if (checkDefault)
            {
                ...
                {
                    // Make sure we have a default site
                    var count = await conn.ExecuteScalarAsync<int>($"SELECT COUNT(*) FROM [{table}] WHERE [IsDefault]=1 AND [Id]!=@Id", new
                    {
                        Id = model.Id
                    });
                    if (count == 0)
            ...
            await conn.ExecuteAsync($"UPDATE [{table}] SET [InternalId]=@InternalId, [Title]=@Title, [Description]=@Description, [Hostnames]=@Hostnames, [IsDefault]=@IsDefault, [LastModified]=@LastModified WHERE [Id]=@Id",
                model, transaction: transaction);
        }
        ...
        /// <returns>The sitemap</returns>
        private async Task<Models.Sitemap> Sort(IEnumerable<Page> pages, string parentId = null, int level = 0)
        {
            ...
            foreach (var page in pages.Where(p => p.ParentId == parentId).OrderBy(p => p.SortOrder))
            {
                ...
                item.Level = level;
                item.Items = await Sort(pages, page.Id, level + 1);
