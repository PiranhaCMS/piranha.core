...
namespace Piranha.AspNetCore.Models
{
    public class SinglePostModel<T> : Microsoft.AspNetCore.Mvc.RazorPages.PageModel where T : PostBase
    {
        ...
        public SinglePostModel(IApi api, IAuthorizationService auth)
        {
            _api = api;
            _auth = auth;
        }
        ...
        public virtual async Task<IActionResult> OnGet(Guid id, bool draft = false)
        {
            ...
            if (draft)
            {
                ...
                if ((await _auth.AuthorizeAsync(HttpContext.User, Security.Permission.PostPreview)).Succeeded)
                {
                    // Get the draft, if available
                    Data = await _api.Posts.GetDraftByIdAsync<T>(id);

                    if (Data == null)
                    {
                        Data = await _api.Posts.GetByIdAsync<T>(id);
                    }
                }
            }
