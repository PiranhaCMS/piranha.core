...
- [x] **Task: Create `test/Aero.Cms.Manager.IntegrationTests` project** [f0fe306]
    - [x] Create `.csproj` with references to `MvcWeb`, `Alba`, `xunit`, `Raven.TestDriver`.
- [x] **Task: Set up `AlbaHost` for Integration Testing** [7df246a]
    - [x] Create a custom `AlbaHost` setup in the test project that targets `MvcWeb`.
    - [x] Configure the host to use a test RavenDB store (Raven.TestDriver) and ensure `IdentityWithSeed` is called.
...
- [x] **Task: Test Admin Login with Alba `FormData`** [bf58fc4]
    - [x] Execute `Seed()` within the test host initialization or at the start of the test.
    - [x] Implement a test using `host.Scenario(...)`.
    - [x] Use `_.Post.FormData(loginModel).ToUrl("/manager/login")` to simulate the login POST.
    - [x] Verify the response status code using `_.StatusCodeShouldBe(302)` (redirection to `/manager` or the auth endpoint).
