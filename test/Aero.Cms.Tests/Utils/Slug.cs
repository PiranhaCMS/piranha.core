...
public class Slug
{
    ...
    public void ToLowerCase() {
        Assert.Equal("mycamelcasestring", Aero.Cms.Utils.GenerateSlug("MyCamelCaseString"));
    }

    [Fact]
    public void Trim() {
        Assert.Equal("trimmed", Aero.Cms.Utils.GenerateSlug(" trimmed  "));
    }

    [Fact]
    public void ReplaceWhitespace() {
        Assert.Equal("no-whitespaces", Aero.Cms.Utils.GenerateSlug("no whitespaces"));
    }

    [Fact]
    public void ReplaceDoubleDashes() {
        Assert.Equal("no-whitespaces", Aero.Cms.Utils.GenerateSlug("no - whitespaces"));
        Assert.Equal("no-and-whitespaces", Aero.Cms.Utils.GenerateSlug("no & whitespaces"));
    }
    ...
    public void ReplaceLatinCharacters() {
        ...
        foreach (var kvp in charMap)
        {
            Assert.Equal($"foo-{kvp.Value.ToLower()}-bar-baz",
                Aero.Cms.Utils.GenerateSlug($"foo {kvp.Key} bar baz"));
        }
    }
    ...
    public void RemoveSlashesForNonHierarchical() {
        Assert.Equal("no-more-dashes", Aero.Cms.Utils.GenerateSlug("no/more / dashes", false));
    }
}
